...
import type { PredictBridgeMethods, PredictiveIntentConfig, Insight, ActionRate, InsightFeedback } from "./predictive/types.js";
...
const cortexPlugin = {
  ...
  configSchema: Type.Object({
    ...
    predictive_intent: Type.Optional(Type.Object({
      enabled: Type.Boolean({ default: true }),
      debug: Type.Boolean({ default: false }),
    })),
  }),
  ...
  register(api: OpenClawPluginApi) {
    ...
    const predictiveRawConfig = (rawConfig as Record<string, unknown>)?.predictive_intent as Record<string, unknown> | undefined;
    ...
    if (predictEnabled) {
      ...
      const predictConfig: PredictiveIntentConfig = {
        enabled: true,
        poll_intervals_ms: {
          'augur.signals': 60000, 'augur.trades': 300000, 'augur.regime': 300000,
          'augur.paper': 900000, 'git.activity': 600000, 'fleet.health': 300000,
          'octoprint.jobs': 300000, 'pipeline.state': 120000, 'cortex.session': 0,
          'cortex.atoms': 600000,
        },
        staleness_thresholds_ms: {
          'augur.signals': 120000, 'augur.trades': 600000, 'augur.regime': 600000,
          'augur.paper': 1800000, 'git.activity': 1200000, 'fleet.health': 600000,
          'octoprint.jobs': 600000, 'pipeline.state': 240000, 'cortex.session': 30000,
          'cortex.atoms': 1200000,
        },
        urgency_thresholds: { high: 0.60, critical: 0.85 },
        delivery: {
          signal_channel: 'signal',
          focus_detection_window_ms: 90000, focus_detection_min_calls: 3,
          batch_window_ms: 300000, duplicate_window_ms: 3600000,
        },
        anomaly_thresholds: {
          augur_signal_stale_ms: 300000, augur_loss_streak: 3,
          augur_pnl_loss_pct: 0.02, fleet_ssh_timeout_ms: 5000, pipeline_stuck_ms: 3600000,
        },
        feedback: {
          action_window_ms: 600000, rate_increase_per_act: 0.1,
          rate_decrease_per_ignore: 0.05, min_observations: 20, low_value_threshold: 0.10,
        },
        briefings: {
          morning_hour_est: 6, pre_sleep_idle_ms: 5400000, suppression_window_ms: 14400000,
        },
        octoprint: {
          host: 'http://192.168.10.141', secrets_file: '~/.secrets/octoprint.env',
        },
      ...
      const predictBridge: PredictBridgeMethods = {
        ...
        getFeedbackHistory: async (sourceId, insightType, actedOn, windowDays) => {
          const rows = await bridge.predictGetFeedbackHistory(sourceId, insightType, actedOn, windowDays);
      ...
      pollingEngine.registerAdapter(new OctoPrintAdapter(
      ...
      pollingEngine.registerAdapter(new PipelineAdapter(
    ...
    api.registerTool(
      {
        ...
        description: "Query the Predictive Intent system for current insights. Returns scored, queued, or recently delivered insights relevant to the given context.",
        parameters: Type.Object({
          ...
          urgency_min: Type.Optional(Type.String({ description: "Minimum urgency: low, medium, high, critical" })),
          include_queue: Type.Optional(Type.Boolean({ description: "Include queued (undelivered) insights" })),
        ...
        async execute(_toolCallId, params) {
          ...
          const p = params as {
            ...
            urgency_min?: string; include_queue?: boolean; limit?: number;
          };
          const result = pollingEngine.queryInsights(p);
          const formatted = result.insights.length === 0
